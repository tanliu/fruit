...
var updataUrl = baseUrl + "/user/editor";
var updatePwdUrl=baseUrl+"/user/editorPwd";
    ...
    return [
        ...
        },
        {
            "render": function (data, type, full, meta) {
                ...
                operation = operation + "  <a href=\"javascript:reSetPwd(\'" + full.userId + "\');\"><span title='重置密码' class='fa fa-lock' aria-hidden='true'></span></a>"
...
function addCheckBox(param) {
    ...
    for (var i = 0; i < param.length; i++) {
        ...
        var $check = $("<div class=\"checkbox\"><label><input type=\"checkbox\" id=\'" + param[i].roleId + "\' class=\"flat\"  value=\'" + param[i].roleId + "\' name='roleIds'> " + param[i].roleName + "</label></div>");

        /*        var $check=$("<input type='checkbox' id=\'"+param[i].roleId+"\' value=\'"+param[i].roleId+"\' name='roleIds'/>");
         $check.html(param[i].roleName);*/
...
function setCheckRole(userRoles) {
    for (var i = 0; i < userRoles.length; i++) {
        $("#" + userRoles[i].role.roleId).attr("checked", "checked");
    }
}

function reSetPwd(id) {

    opneModel({url: PwdHtmlUrl, onOK: editPwd, title: editPwdTile});
    $('#myModal').on('shown.bs.modal', function () {
        init_validator();
    });
}
