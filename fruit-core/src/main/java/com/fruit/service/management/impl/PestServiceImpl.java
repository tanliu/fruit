...
import com.fruit.service.management.PestService;
 ...
 */
@Service
public class PestServiceImpl extends BaseServiceImpl<Pest> implements PestService {
    PestDao pestDao;

    @Resource(name = PestDao.DAO_NAME)
    public void setPestDao(PestDao pestDao) {
        super.setDaoSupport(pestDao);
    ...
    public String getPersonDetail(Integer id){
        String sql = "select t.id,t.drugname,t.name,t.measure,DATE_FORMAT(t.date, '%Y-%m-%d') date,t.number,t.pictures from pest t where id=?";
    ...
    public String showRecords(Integer page,Integer pageSize,int id,Map<String, String> params){
        ...
        sql.append("((( select f.id,f.drugname,f.name,f.measure,DATE_FORMAT(f.date, '%Y-%m-%d') date,f.pictures,p.orchardId,p.varietyId,p.producingAreaId  ");
        sql.append(" from pest f left join productinformation p on f.productInformationId=p.id where f.farmerId=? ").append(condition.toString());
    ...
    public String showRecordsByAdmin(Integer page,Integer pageSize,Integer id,Map<String, String> params){
        ...
        sql.append("select t.*,concat(v.name,'(',pr.name,')') vname,o.name oname from ");
        sql.append("((( select f.id,f.drugname,f.name,f.measure,DATE_FORMAT(f.date, '%Y-%m-%d') date,f.pictures,p.orchardId,p.varietyId,p.producingAreaId  ");
        sql.append(" from pest f left join productinformation p on f.productInformationId=p.id where p.companyId=? ").append(condition.toString());
        sql.append(" order by f.date desc) t  left join variety v on t.varietyId=v.id) left join orchard o on t.orchardId=o.id) ").append(ReginoCondition.toString());
        sql.append(" left join producingarea pr on t.producingAreaId=pr.id ");
    ...
    public String getPestDetail(int id){
        StringBuffer sql = new StringBuffer();
        sql.append("select t.id,t.name,t.measure,t.pictures,t.drugname,DATE_FORMAT(t.date, '%Y-%m-%d') date from pest t where id=?");
...
//	}
    ...
    public String showRecordsForAndroid(Integer page,Integer pageSize,int id,Map<String, String> params){
        ...
        StringBuffer sql = new StringBuffer();
        sql.append("select t.*,concat(v.name,'(',pr.name,')') vname,o.name oname from ");
        sql.append("((( select f.id,f.drugname,f.name,f.measure,DATE_FORMAT(f.date, '%Y-%m-%d') date,f.pictures,p.orchardId,p.varietyId,f.productInformationId productId,p.producingAreaId  ");
        sql.append(" from pest f left join productinformation p on f.productInformationId=p.id where f.farmerId=? ").append(condition.toString());
        sql.append(" order by f.date desc) t  left join variety v on t.varietyId=v.id) left join orchard o on t.orchardId=o.id) ");
        sql.append(" left join producingarea pr on t.producingAreaId=pr.id ");
    ...
    public List<Pest> getMoreRecord(Integer productId,Integer skip,Integer size){
        ...
        String hql = "from Pest where productinformation.id="+productId+" order by date desc";
    ...
    public String getMoreRecordByMobile(Integer productId,Integer skip,Integer size){
        ...
        if(productId==null) productId=-1;
        StringBuffer sql = new StringBuffer();
        sql.append("select id,farmerId,measure,name,drugname,number,pictures,DATE_FORMAT(date, '%Y-%m-%d') date ");
        sql.append(" from pest where productInformationId=? ORDER BY date desc limit ?,?");
