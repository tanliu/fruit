...
public class QualityinspectorServiceImpl  extends BaseServiceImpl<Qualityinspector> implements QualityinspectorService{
    ...
    public String showInspectors(Integer page,Integer pageSize,Integer companyid,Map<String, String> params){
        ...
        sql.append("select id,name,username,phone,address,qq,email,DATE_FORMAT(contractStart, '%Y-%m-%d') contractStart,DATE_FORMAT(contractEnd, '%Y-%m-%d') contractEnd,DATE_FORMAT(createTime, '%Y-%m-%d %H:%i:%s') createTime from qualityinspector t  where companyId=? ");
    ...
    public Employee getEmployee(String employNo) {
        ...
        List<Qualityinspector> qualityinspectors = findObjectByFields(fields, params);
        Employee employee=qualityinspectors.get(0);
        return employee;
    }
