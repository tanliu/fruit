...
import com.fruit.service.management.IrrigationService;
 ...
 */
@Service
public class IrrigationServiceImpl extends BaseServiceImpl<Irrigation> implements IrrigationService{

    IrrigationDao irrigationDao;

    @Resource(name = IrrigationDao.DAO_NAME)
    public void setIrrigationDao(IrrigationDao irrigationDao) {
        super.setDaoSupport(irrigationDao);
    ...
    public String getPersonDetail(Integer id){
        String sql = "select t.id,t.water,t.way,t.watersource,DATE_FORMAT(t.date, '%Y-%m-%d') date,t.number,t.pictures from irrigation t where id=?";
    ...
    public String showRecords(Integer page,Integer pageSize,int id,Map<String, String> params){
        ...
        sql.append("((( select f.id,f.water,f.way,f.watersource,DATE_FORMAT(f.date, '%Y-%m-%d') date,f.pictures,p.orchardId,p.varietyId,p.producingAreaId  ");
        sql.append(" from irrigation f left join productinformation p on f.productInformationId=p.id where f.farmerId=? ").append(condition.toString());
    ...
    public String showRecordsByAdmin(Integer page,Integer pageSize,Integer id,Map<String, String> params){
        ...
        sql.append("select t.*,concat(v.name,'(',pr.name,')') vname,o.name oname from ");
        sql.append("((( select f.id,f.water,f.way,f.watersource,DATE_FORMAT(f.date, '%Y-%m-%d') date,f.pictures,p.orchardId,p.varietyId,p.producingAreaId  ");
        sql.append(" from irrigation f left join productinformation p on f.productInformationId=p.id where p.companyId=? ").append(condition.toString());
        sql.append(" order by f.date desc) t  left join variety v on t.varietyId=v.id) left join orchard o on t.orchardId=o.id) ");
    ...
    public String getIrrigationDetail(int id){
        StringBuffer sql = new StringBuffer();
        sql.append("select t.id,t.way,t.water,t.pictures,t.watersource,DATE_FORMAT(t.date, '%Y-%m-%d') date from irrigation t where id=?");
...
//	}
    ...
    public String showRecordsForAndroid(Integer page,Integer pageSize,int id,Map<String, String> params){
        ...
        StringBuffer sql = new StringBuffer();
        sql.append("select t.*,concat(v.name,'(',pr.name,')') vname,o.name oname from ");
        sql.append("((( select f.id,f.water,f.way,f.watersource,DATE_FORMAT(f.date, '%Y-%m-%d') date,f.pictures,p.orchardId,p.varietyId,f.productInformationId productId,p.producingAreaId  ");
        sql.append(" from irrigation f left join productinformation p on f.productInformationId=p.id where f.farmerId=? ").append(condition.toString());
        sql.append(" order by f.date desc) t  left join variety v on t.varietyId=v.id) left join orchard o on t.orchardId=o.id) ");
        sql.append(" left join producingarea pr on t.producingAreaId=pr.id ");
    ...
    public List<Irrigation> getMoreRecord(Integer productId,Integer skip,Integer size){
        ...
        String hql = "from Irrigation where productinformation.id="+productId+" order by date desc";
    ...
    public String getMoreRecordByMobile(Integer productId,Integer skip,Integer size){
        ...
        if(productId==null) productId=-1;
        StringBuffer sql = new StringBuffer();
        sql.append("select id,farmerId,watersource,way,water,number,pictures,DATE_FORMAT(date, '%Y-%m-%d') date ");
        sql.append(" from irrigation where productInformationId=? ORDER BY date desc limit ?,?");
